...
--
-- 파일명은 Oracle 이 정한다 -- 넘긴 접두사 뒤에 워커 id 와 타임스탬프를 붙인다.
-- 그래서 재내보내기는 덮어쓰지 않고 쌓이므로, 먼저 해당 프리픽스를 비운다.
--
-- 접두사에 날짜를 넣어 파일명만으로 어느 날 것인지 알 수 있게 한다. 경로에
-- 이미 있어 중복이지만, 경로는 내려받는 순간 사라진다 -- 여러 날 것을 한
-- 폴더에 모아 두면 part_ 로 시작하는 이름들은 서로 구분되지 않는다.
--
-- 날짜를 폴더로도 유지하는 이유는 나누어 담으려는 게 아니라 지울 단위여서다.
...
-- 건)를 리스팅해 이름으로 걸러야 한다. 하루를 지우고 다시 쓰는 것이 이
-- 프로시저의 멱등성 전부라, 그 경계를 파일명에 맡기지 않는다.
--
  ...
  WHILE v_day <= v_to LOOP
    ...
    IF v_rows > 0 THEN
      ...
      DBMS_CLOUD.EXPORT_DATA(
        ...
        file_uri_list   => v_prefix || v_day || '_part',
